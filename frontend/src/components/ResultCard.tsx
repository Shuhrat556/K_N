    ...
    <motion.div
      ...
      className={[
        ...
        highlight
          ...
          : "bg-white/85 text-ink-900 ring-slate-200/70",
      ].join(" ")}
    >
      <div className="flex items-start gap-4">
        <div
          className={[
            ...
            highlight ? "bg-white/15 ring-1 ring-white/25" : "bg-slate-900 text-white",
          ].join(" ")}
        >
          {icon}
        </div>
        <div className="min-w-0 flex-1">
          <div className={["text-xs font-bold uppercase tracking-wide", highlight ? "text-white/80" : "text-ink-500"].join(" ")}>
            {badge ?? (highlight ? "Главный результат" : "Детали")}
          </div>
          <div
            className={["mt-1 text-xl font-extrabold leading-snug sm:text-lg", highlight ? "text-white" : "text-ink-900"].join(" ")}
          >
            {title}
          </div>
          {subtitle ? (
            <div
              className={[
                ...
                highlight ? "text-white/85" : "text-ink-700",
              ].join(" ")}
            >
              {subtitle}
            </div>
