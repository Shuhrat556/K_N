    ...
    return (
      <motion.button
        ...
        className={[
          ...
          active ? "bg-slate-900 text-white ring-slate-900" : "bg-white/80 text-ink-800 ring-slate-200/70",
        ].join(" ")}
      >
        {label}
      </motion.button>
  ...
  return (
    <div className="page-shell flex min-h-full flex-col pb-16 pt-6 sm:pt-8">
      <header className="flex items-center justify-between gap-4">
        <div>
          <div className="text-base font-extrabold text-ink-900 sm:text-sm">{t(lang, "universities_title")}</div>
          <div className="mt-1 text-sm font-semibold text-ink-600 sm:text-xs">{t(lang, "universities_sub")}</div>
        </div>
      ...
      <section className="mt-8 rounded-3xl bg-white/80 p-6 shadow-soft ring-1 ring-slate-200/70">
        <div className="grid gap-5 lg:grid-cols-2">
          <div>
            <div className="text-xs font-black uppercase tracking-wide text-ink-500">{t(lang, "filter_location")}</div>
            <div className="mt-3 flex flex-wrap gap-2">
              {(["all", "dushanbe", "moscow", "remote", "bokhtar", "khujand"] as const).map((v) => chip(v, loc, setLoc, locLabel(lang, v)))}
            </div>
          </div>
          <div>
            <div className="text-xs font-black uppercase tracking-wide text-ink-500">{t(lang, "filter_district")}</div>
            <div className="mt-3 flex flex-wrap gap-2">
              {(["all", "center", "north", "south", "remote"] as const).map((v) => chip(v, dist, setDist, distLabel(lang, v)))}
            </div>
          </div>
          <div>
            <div className="text-xs font-black uppercase tracking-wide text-ink-500">{t(lang, "filter_type")}</div>
            <div className="mt-3 flex flex-wrap gap-2">
              {(["all", "full-time", "part-time", "online"] as const).map((v) => chip(v, study, setStudy, studyLabel(lang, v)))}
            </div>
          </div>
          <div>
            <div className="text-xs font-black uppercase tracking-wide text-ink-500">{t(lang, "filter_lang")}</div>
            <div className="mt-3 flex flex-wrap gap-2">
              {(["all", "ru", "tj", "en", "mixed"] as const).map((v) => chip(v, langF, setLangF, langLabel(lang, v)))}
            </div>
          </div>
          <div>
            <div className="text-xs font-black uppercase tracking-wide text-ink-500">{t(lang, "filter_price")}</div>
            <div className="mt-3 flex flex-wrap gap-2">
      ...
      <div className="mt-8 grid gap-4 lg:grid-cols-2">
        {filtered.map((u, i) => (
          <motion.article
            ...
            className="rounded-3xl bg-white/85 p-6 shadow-soft ring-1 ring-slate-200/70"
          >
            <div className="flex items-start justify-between gap-4">
              <div>
                <div className="text-lg font-extrabold text-ink-900">{u.name}</div>
                <div className="mt-1 text-xs font-semibold text-ink-600">
                  {t(lang, "uni_faculty")}: {u.faculty}
                </div>
                <div className="mt-2 flex flex-wrap gap-2 text-[11px] font-bold">
                  <span className="rounded-full bg-slate-900/5 px-2 py-1 text-ink-800">{locLabel(lang, u.location as Loc)}</span>
                  <span className="rounded-full bg-slate-900/5 px-2 py-1 text-ink-800">{distLabel(lang, u.district as Dist)}</span>
                  <span className="rounded-full bg-slate-900/5 px-2 py-1 text-ink-800">{studyLabel(lang, u.studyType)}</span>
                  <span className="rounded-full bg-slate-900/5 px-2 py-1 text-ink-800">{langLabel(lang, u.language as LangF)}</span>
                  <span className="rounded-full bg-slate-900/5 px-2 py-1 text-ink-800">{priceLabel(lang, u.price)}</span>
                </div>
              </div>
            ...
            <div className="mt-4 text-xs font-semibold text-ink-600">Demo sort: higher score = stricter entry (as requested).</div>
          </motion.article>
        ))}
      </div>
    </div>
