...
const excel_parser_js_1 = require("../services/excel-parser.js");
...
router.post("/api/upload-excel/preview", upload.single("file"), async (req, res) => {
    try {
        ...
        const preview = (0, excel_parser_js_1.previewExcel)(file.buffer, 10);
        res.json({
            ok: true,
...
router.post("/api/upload-excel", upload.single("file"), async (req, res) => {
    try {
        ...
        const rows = (0, excel_parser_js_1.parseExcelBuffer)(file.buffer);
        if (!rows.length) {
        ...
        for (const row of rows) {
            try {
                ...
                await prisma_js_1.prisma.specialty.upsert({
                    where: {
                        specialty_unique: {
                            code: row.code,
